...
We use four simulated datasets for demonstration. `A`, `B` are two datasets generated through similar second-order model, with same mean, variance and loadings for each shared items. They each include distinct cohort `f1a` and `f1b` respectively, and share two cohort `f2` and `f3`.

```{r, echo=FALSE, out.width="100%", fig.align='center', fig.cap="example simulation"}
knitr::include_graphics("images/simulation_graph.png")
```

In addition, `BUNI` and `BNONUNI` are adaptations of `B` by adding uniform/nonuniform DIF onto each items in `f3`.
...
final_res <- harmonization(sim_model, unidif, nonunidif)
...
# Combine into one df
all_scores <- do.call(rbind, scores_list)

# plot density of factor scores for each cohort
library(ggplot2)
plot_cols <- setdiff(names(all_scores), "cohort")
for (col in plot_cols) {
  p <- ggplot(all_scores, aes(x = .data[[col]], fill = as.factor(cohort))) +
