#' Fit a `harm_model` into Second Order Linear Models
#'
#' Given full model information and selected items and groups, for each group,
#' fit the data with a individual second order general factor linear model.
...
#' @param std Logical. If true standardize general factor. Default to `TRUE`.
#'
#' @return a fit result, or a named list of fit results with `lavaan::sem`
#' @export

fit2nd <- function(model, selected_items, selected_cohorts, std=TRUE){
    ...
    # --- In case of selected_cohorts being a character
