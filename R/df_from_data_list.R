...
df_from_data_list <- function(model, target_items, target_groups) {
    ...
    # check type of target_groups
    if (!is.list(target_groups) && !is.character(target_groups)) {
        stop("`target_groups` must be a character or list", call. = FALSE)
    }

    # check that the groups themselves exist in the list
    ...
    if (is.character(target_groups) || length(target_groups) == 1) {
        ...
        if (length(missing_items_in_group) > 0) {
            ...
            stop(paste0(
                "Validation failed. Required items were missing from target group:\n",
                msg))
        }

        df <- model$data[[group_name]]
        return(df[, target_items])
    }
